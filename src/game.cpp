  ...
  mPlayerContext2(std::make_shared<PlayerContext>())
{
  ...
  mFont = TTF_OpenFont("romfs:/font.ttf", 16);
  if (mFont == nullptr) {
  ...
  mSpriteSheet = Image::fromFile(mRenderer, "romfs:/spritesheet.png");
    ...
    while (SDL_PollEvent(&event) != 0) {
      ...
      case SDL_JOYBUTTONDOWN:
        if (event.jbutton.button == 11) // MINUS
          ...
          mScene->onKeyDown(event.jbutton);
        break;
      case SDL_JOYBUTTONUP:
        mScene->onKeyUp(event.jbutton);
        break;
      }
    }
